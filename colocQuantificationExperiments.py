...
for idx, row in df.iterrows():
    ...
    tumor=RCM*tumor
    amountoftumor0=np.prod(np.where(tumor)[0].shape)
    ...
    #CQM= np.zeros_like(CD44v6DAB,dtype="float")
    #green= np.zeros_like(CD44v6DAB,dtype="float")
    #blue= np.zeros_like(CD44v6DAB,dtype="float")
#
    #CQM[EcadDAB[:,:]>0]+=1.0
    #green[CD44v6DAB[:,:]>0]+=1.0
#
    #CQM=np.stack((CQM,green,blue),axis=-1)
#
    ##multiply with the RCM coming from the intersection of Hs
    #CQM=CQM*np.stack((RCMxSegTum,RCMxSegTum,RCMxSegTum),axis=-1)
    ...
    #We no longer need the amount tissue, we want amounttumour
    # but we can still calculate this :)
    # might be interesting to find how much tumour 
    # thereis with respect to all tissue
    
    #amounttissue0=np.prod(np.where(RCM)[0].shape)
    #segtum=None


    
df.to_csv(mainloc+"N261DAB-oldtumor-trans-02-015-1-tumor-v6.csv")
